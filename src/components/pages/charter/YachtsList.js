...
const YachtsList = ({ yachtTypeId, title }) => {
    ...
    useEffect(() => {
        const fetchYachts = async () => {
            try {
                let url = "https://adriaticsailingadventure.com/backend/wp-json/wp/v2/yacht";
                
                // If yachtType is provided, filter by taxonomy
                if (yachtTypeId) {
                    url += `?yacht_type=${yachtTypeId}`;
                }
        ...
        fetchYachts();
    }, [yachtTypeId]); // Refetch when yachtType changes
            ...
            <ul style={{
                ...
                {yachts.map((yacht) => (
                    <li key={yacht.id} style={{
                        ...
                        <Link to={`/yachts/${yacht.id}`} target="_blank" rel="noopener noreferrer">
                            <MediaImg 
                        ...
                        <p>
                            ...
                            <strong>Lowest Price:</strong> {yacht.acf.lowest_weekly_price || 'N/A'}
                        </p>
                        <p>
